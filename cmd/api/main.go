...
func setApi(config Config) {
	...
	v1.GET("/user/:id", handlers.AuthenticateRequest(), handlers.AuthorizeRequest(config.ruler), config.userHandler.Get)
	v1.POST("/user/", handlers.AuthenticateRequest(), handlers.AuthorizeRequest(config.ruler), config.userHandler.Create)
	v1.GET("/user/drivers", handlers.AuthenticateRequest(), handlers.AuthorizeRequest(config.ruler), config.userHandler.GetDrivers)

	v1.GET("/travel/:id", handlers.AuthenticateRequest(), handlers.AuthorizeRequest(config.ruler), config.travelHandler.Get)
	v1.PUT("/travel/:id", handlers.AuthenticateRequest(), handlers.AuthorizeRequest(config.ruler), config.travelHandler.Edit)
	v1.POST("/travel/", handlers.AuthenticateRequest(), handlers.AuthorizeRequest(config.ruler), config.travelHandler.Create)
